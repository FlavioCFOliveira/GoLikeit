...
name: go-gitflow
description: |
  Safe Gitflow for Go with mandatory human approval, detailed technical commit messages, and STRICT git push requirements.

  TRIGGER when:
  - User mentions "git", "branch", "merge", "commit", "push", "pull", "checkout", "tag", "stash", "rebase", "cherry-pick"
  - User asks to "create a feature", "start a feature", "finish a feature", "complete a feature"
  - User mentions "hotfix", "release", "version", "tag", "deploy"
  - User wants to "merge" changes, "delete branch", "switch branch"
  - User asks about git workflow, branching strategy, or git operations
  - User mentions gitflow, GitFlow, or git-flow patterns
  - User needs to commit code, push changes, or pull updates
  - User mentions "uncommitted changes", "conflicts", "git status", "git log"

  ALWAYS use this skill for ANY git operation in a Go project, even if user doesn't explicitly say "use gitflow".
commands:
  - name: /feature-start
...
1. [Approval Policy (MANDATORY)](#1-approval-policy-mandatory)
2. [Detailed Commit Standards](#2-detailed-commit-standards)
3. [Feature Start (`/feature-start`)](#3-feature-start-feature-start-name)
4. [Feature Finish (`/feature-finish`)](#4-feature-finish-feature-finish)
5. [Hotfix Procedure (`/hotfix`)](#5-hotfix-procedure-hotfix-version)
6. [Release Procedure (`/release-start`)](#6-release-procedure-release-start-version)
7. [Release Finish (`/release-finish`)](#7-release-finish-release-finish-version)
8. [Visual Workflow Diagrams](#8-visual-workflow-diagrams)
9. [Rollback Procedures](#9-rollback-procedures)
10. [Collaborative Ecosystem](#10-collaborative-ecosystem)
11. [Roadmap Integration](#11-roadmap-integration)
12. [Error Handling](#12-error-handling)
13. [Quick Command Reference](#13-quick-command-reference)
14. [Basic Git Commands](#14-basic-git-commands)
...
```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│    main     │────▶│ hotfix/vX.Y │────▶│    main     │
│  (production)│    │ (fix here)  │     │  (tagged)   │
└─────────────┘     └─────────────┘     └─────────────┘
                            │                   │
                            ▼                   ▼
                      ┌─────────────┐     ┌─────────────┐
...
```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│   develop   │────▶│ release/vX  │────▶│    main     │
│ (features)  │     │(stabilize)  │     │  (release)  │
└─────────────┘     └─────────────┘     └──────┬──────┘
       ▲                                       │
       │              ┌─────────────┐          │
       └──────────────│   MERGE     │◀─────────┘
