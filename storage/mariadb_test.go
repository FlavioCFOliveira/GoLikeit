...
// skipIfNoMariaDB skips the test if MariaDB is not available.
func skipIfNoMariaDB(t *testing.T) {
	// For CI environments with MariaDB service
	// Set MARIADB_TEST_DSN environment variable
	dsn := "user:password@tcp(localhost:3306)/golikeit_test?parseTime=true"
	if dsn == "" {
		t.Skip("Skipping MariaDB test: MARIADB_TEST_DSN not set")
	}
}

// newTestMariaDBStorage creates a test MariaDB storage.
func newTestMariaDBStorage(t *testing.T) *MariaDBStorage {
	skipIfNoMariaDB(t)

	config := MariaDBConfig{
		ConnectionString: "user:password@tcp(localhost:3306)/golikeit_test?parseTime=true",
		MaxOpenConns:     5,
		MaxIdleConns:     2,
	}
	...
	// Clean up test data
	storage.db.ExecContext(ctx, "TRUNCATE TABLE reactions")

	return storage
}

func TestMariaDBStorage_AddReaction(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_RemoveReaction(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetUserReaction(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_HasUserReaction(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetEntityCounts(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetUserReactions(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetUserReactionCounts(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetUserReactionsByType(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetEntityReactions(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetRecentReactions(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetLastReactionTime(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBStorage_GetEntityReactionDetail(t *testing.T) {
	storage := newTestMariaDBStorage(t)
	defer storage.Close()
...
func TestMariaDBConfig_Default(t *testing.T) {
	config := DefaultMariaDBConfig()
