...
func skipIfNoMongoDB(t *testing.T) {
	// For CI environments with MongoDB service
	// Set MONGODB_TEST_URI environment variable
	uri := "mongodb://localhost:27017"
	if uri == "" {
		t.Skip("Skipping MongoDB test: MONGODB_TEST_URI not set")
	}
}

// newTestMongoDBStorage creates a test MongoDB storage.
func newTestMongoDBStorage(t *testing.T) *MongoDBStorage {
	skipIfNoMongoDB(t)

	config := MongoDBConfig{
		URI:             "mongodb://localhost:27017",
		Database:        "golikeit_test",
		Collection:      "reactions_test",
		MaxPoolSize:     10,
		MinPoolSize:     2,
	}

	ctx := context.Background()
	storage, err := NewMongoDBStorage(ctx, config)
	if err != nil {
	...
	// Clean up test data
	storage.collection.Drop(ctx)

	return storage
}

func TestMongoDBStorage_AddReaction(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_RemoveReaction(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetUserReaction(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_HasUserReaction(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetEntityCounts(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetUserReactions(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetUserReactionCounts(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetUserReactionsByType(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetEntityReactions(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetRecentReactions(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetLastReactionTime(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBStorage_GetEntityReactionDetail(t *testing.T) {
	storage := newTestMongoDBStorage(t)
	defer storage.Close()
...
func TestMongoDBConfig_Default(t *testing.T) {
	...
	if config.Database != "golikeit" {
		t.Errorf("Expected database golikeit, got %s", config.Database)
	}
}
