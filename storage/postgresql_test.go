...
func skipIfNoPostgres(t *testing.T) {
	// For CI environments with PostgreSQL service
	// Set POSTGRES_TEST_DSN environment variable
	dsn := "postgres://postgres:postgres@localhost:5432/golikeit_test?sslmode=disable"
	if dsn == "" {
		t.Skip("Skipping PostgreSQL test: POSTGRES_TEST_DSN not set")
	}
}

// newTestPostgresStorage creates a test PostgreSQL storage.
func newTestPostgresStorage(t *testing.T) *PostgreSQLStorage {
	skipIfNoPostgres(t)

	config := PostgreSQLConfig{
		ConnectionString: "postgres://postgres:postgres@localhost:5432/golikeit_test?sslmode=disable",
		MaxConns:         5,
		MinConns:         1,
	}
	...
	// Clean up test data
	storage.pool.Exec(ctx, "TRUNCATE TABLE reactions")

	return storage
}

func TestPostgreSQLStorage_AddReaction(t *testing.T) {
...
func TestPostgreSQLStorage_RemoveReaction(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetUserReaction(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_HasUserReaction(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetEntityCounts(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetUserReactions(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetUserReactionCounts(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetUserReactionsByType(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetEntityReactions(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetRecentReactions(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
	...
	if recent[0].UserID != "user2" {
		t.Errorf("Expected user2 first (most recent), got %s", recent[0].UserID)
	}
}

func TestPostgreSQLStorage_GetLastReactionTime(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLStorage_GetEntityReactionDetail(t *testing.T) {
	storage := newTestPostgresStorage(t)
	defer storage.Close()
...
func TestPostgreSQLConfig_Default(t *testing.T) {
